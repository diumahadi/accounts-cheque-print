...
public class InvSubContractorBillPaidDao {

    private final Connection connection;

    public InvSubContractorBillPaidDao() {
        this.connection = DbUtil.getConnection();
    }

    public void save(InvBillPaid data, Connection conn) {
        try {
            String sql = "INSERT INTO inv_subcontruct_bill_paid (subcontractor,payment_project,pay_mode,cheque_no,cheque_date,amount,ait_payable,pay_date,paied_by) VALUES (?,?,?,?,?,?,?,?,?)";
            ...
            try {
                ...
                System.out.println("Error...:InvSubContractorBillPaidDao...:insertm...rollback" + e1);
            }
    ...
    public boolean delete(Long chequeId,Connection conn) {
        try {
            String sql = "DELETE FROM inv_subcontruct_bill_paid WHERE cheque_no = ?";
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public InvBillPaid find(String id) {
        ...
        try {
            String sql = "SELECT * FROM inv_subcontruct_bill_paid WHERE bill_paid_id = ?";
            ...
            System.out.println("InvSubContractorBillPaidDao:Find:SQL Error........." + e);
        }
        return data;
    }
    ...
    public List<InvBillPaid> getPaymentsUndearCheque(Long chequeId){     
        ...
        try {
            String sql = "SELECT * FROM inv_subcontruct_bill_paid WHERE cheque_no=?";
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setString(1,String.valueOf(chequeId));
            ResultSet rs =  ps.executeQuery();
            while (rs.next()) {
                ...
                data.setProject(projectDao.getProject(rs.getString("payment_project")));
                SupplierDao supplierDao = new SupplierDao();
                data.setSupplier(supplierDao.getSupplier(rs.getInt("subcontractor")));
                list.add(data);
            }
        } catch (SQLException e) {
            System.out.println("InvSubContractorBillPaidDao:getMatch:SQL Error........." + e);
        }
